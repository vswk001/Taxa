...
Taxis ships a read-only **MCP (Model Context Protocol)** server so that AI tools —
Claude Code, Codex, and any MCP-compatible client — can search and read your notes
as a local knowledge base.

It is a separate binary, `taxis-mcp`, that shares the app's storage code and reads
...
Add `taxis` to your MCP config (e.g. `~/.claude.json` → `mcpServers`, or via
...
```jsonc
{
  "mcpServers": {
    "taxis": {
      "command": "/absolute/path/to/taxis-mcp"
    }
  }
}
```
...
**Resources**: `taxis://notes/{id}` → the note's markdown content.
...
- **Concurrent with the app.** Opens the database read-side under WAL; safe to run
  while the Taxis desktop app is open.
- The database must already exist (open the Taxis app once to initialize it).

## Troubleshooting

- *Server exits immediately:* the database wasn't found. Open the Taxis desktop app
